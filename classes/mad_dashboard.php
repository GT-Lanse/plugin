...
class mad_dashboard{
  ...
  public static function enable($courseId)
  {
    ...
    # TODO call backend api to check if this course is enable
    # if it is check on moodle db if it is enable? if it is show get url to se the
    # data.

    $response = self::api_enable_call($courseId, $tokenHex);
    # echo $response;

    # if(is_null($response) || !property_exists($response, 'plugin-info')){
    #   return;
    # }
  ...
  public static function upload_logs($courseId)
  {
    ...
    $s3 = new \S3("foo", "bar", false, "host.docker.internal:4566");
    $logs_query = "
    SELECT  m.id AS Id,
            FROM_UNIXTIME(m.timecreated, '%d/%m/%Y %H:%i') AS 'Hora',
            CONCAT(mu.firstname, ' ' ,mu.lastname) AS 'username',
    ...
    $token = $course_settings->token;
    str_putcsv($logs);
    $s3->putObject(file_get_contents('./temp.csv'),
                   'moodle-logs',
                   "unprocessed/$organization/$campus/$token/$courseId.csv",
                   \S3::ACL_PRIVATE, array(), array('Content-Type' => 'text/csv'));
  }
